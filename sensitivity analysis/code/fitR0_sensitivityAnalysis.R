...
for(scenario in scenarios){
  file_read <- paste0("./sensitivity analysis/outputs/", scenario, "/delta_obs_dat_", scenario, ".xlsx")
  print(file_read)
  
  # import data 
  delta_obs_dat <- read_excel(file_read)
  ...
  for(county in counties){
    ...
    f1 <- function (beta,gamma) {
      ...
      x <- trajectory(sir_frame,params=params, format = "data.frame")
      discrep_df <- data.frame(predicted_sir = x[, c("I")], actual = as.vector(obs(sir_frame)))
      discrep_df$diff <- discrep_df$predicted_sir - discrep_df$actual
      sum(discrep_df$diff^2)
  ...
  write_summary_file <- paste0("./sensitivity analysis/outputs/", scenario, "/delta_county_summary_", scenario, ".xlsx")
  #write_delta_params_file <- paste0("./sensitivity analysis/outputs/", scenario, "/delta_params_", scenario, ".xlsx")
  write_delta_projections_file <- paste0("./sensitivity analysis/outputs/", scenario, "/delta_proj_", scenario, ".xlsx")
  
  
  write_xlsx(delta_summary_dat, write_summary_file)
  ...
  delta_proj_dat%>%
    ...
    geom_line()
  write_xlsx(delta_proj_dat, write_delta_projections_file)
